...
TargetFolder = '/global/cscratch1/sd/lianming/Results/Test'
...
dT1s = np.arange(0, 61, 15)
dT2s =  np.arange(420, 481, 30)
# dT1s = np.arange(60, 76, 15)
# dT2s = np.arange(450, 466, 15)

BinMag = np.arange(-2.25, 4.06, 0.1)
...
#########################################################
#Generating filter pairs, time pairs
...
HashTableDim = [ len(BandPairs), len(dT1s), len(dT2s), len(BinMag)-1, len(BinColor)-1 ]
...
def CalculateMap(BandPair, FilePath,
    ...
    for kk, dT1 in enumerate(dT1s):
        for ll, dT2 in enumerate(dT2s):
            ...
            for II in Objects:
                ...
                TimeRangeStart = max( TimeRange_load[Band1][II][0], TimeRange_load[Band2][II][0] - dT1/1440, TimeRange_load[Band2][II][0] - dT2/1440 )
                ...
                SampleNo = np.int(PointsPerDay*TimeRange)
...
def reduceAndSave(results, EventName, HashTableDim=HashTableDim, 
                  BandPairs=BandPairs, dT1s=dT1s, dT2s=dT2s, 
    ...
    InfoDict = {}
    InfoDict['EventNames'] = EventName
    InfoDict['PointsPerDay'] = PointsPerDay
    InfoDict['Object Number'] = results[0][0][0]
    ...
    if HashTableTotal.min()<0:
        InfoDict['Warning'] = HashTableTotal.min()
